...
import '../../data/remote/ebeasiswa/sp/sp_datasource.dart';
import '../../data/remote/ebeasiswa/sk/sk_datasource.dart';

class LetterOfSPController extends GetxController {

  SPDataSource spDataSource =
      SPDataSource();
    final BoxStorage boxStorage = BoxStorage();

   Future<List<SP>> getUserFund()async{
    String token = boxStorage.getUserToken();
    print('get token : $token');
     final response = boxStorage.getUserName() == 'dewianggreni06@yahoo.co.id' ?  await spDataSource.userSPDummy(token: token): await spDataSource.userSP(token: token);
   
    print('get response : $response');
     if (response['data'] != null) {
      await Future.delayed(const Duration(seconds: 2));
        List<dynamic> parsed =  response['data'];
        List<SP> userfund = parsed.map((i) => SP.fromJson(i)).toList();
      return userfund;
      } else{
