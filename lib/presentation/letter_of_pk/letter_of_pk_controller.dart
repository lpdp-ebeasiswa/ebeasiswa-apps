...
class LetterOfPKController extends GetxController {

  PKDataSource pkDataSource =
      PKDataSource();
    final BoxStorage boxStorage = BoxStorage();

   Future<List<PK>> getUserFund()async{
    String token = boxStorage.getUserToken();
    print('get token : $token');
     final response = boxStorage.getUserName() == 'dewianggreni06@yahoo.co.id' ?  await pkDataSource.userPKDummy(token: token): await pkDataSource.userPK(token: token);
   
    print('get response : $response');
     if (response['data'] != null) {
      await Future.delayed(const Duration(seconds: 2));
        List<dynamic> parsed =  response['data'];
        List<PK> userfund = parsed.map((i) => PK.fromJson(i)).toList();
      return userfund;
      } else{
